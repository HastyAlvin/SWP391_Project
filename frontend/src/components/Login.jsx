...
import { admin_login, customer_login, messageClear, seller_login } from '../store/reducers/authReducer';
...
const Login = () => {
    ...
    const login = (e) => {
        e.preventDefault()
        switch (selectedRole) {
            case 'customer': dispatch(customer_login(state))
                break;
            case 'seller': dispatch(seller_login(state))
                break;
            case 'admin': dispatch(admin_login(state))
                break;
            default:
                navigate('/');
        }
        ...
        useEffect(() => {
            ...
            if (userInfo) {
                if (role === 'admin') {
                    ...
                    window.location.href = 'http://localhost:3000/seller/dashboard';
                  } else {
                                ...
                                <div>
                                    <form onSubmit={login} className='text-slate-600'>
                                        ...
                                        <div className='flex flex-row gap-1 align-middle'>
                                            <div>
                                                <select onChange={(e) => setSelectedRole(e.target.value)} className='w-full px-3 py-2 border border-slate-200 outline-none focus:border-green-500 rounded-md' name="role" id="role" required>
